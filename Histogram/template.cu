...
__global__ void histogram(unsigned int *input, unsigned int *bins,
	...
	unsigned int i = tx + blockIdx.x * blockDim.x;
	unsigned int stride = blockDim.x * gridDim.x;

	for (unsigned int bin = tx; bin < num_bins; bin += blockDim.x) {
		private_histo[bin] = 0;
	}
	...
	for (unsigned int bin = tx; bin < num_bins; bin += blockDim.x) {
		atomicAdd(&(bins[bin]), private_histo[bin]);
	}
...
int main(int argc, char *argv[]) {
  ...
  dim3 satGrid((NUM_BINS + BLOCK_SIZE - 1) / BLOCK_SIZE, 1, 1);
  ...
  saturate<<<satGrid, dimBlock >>> (deviceBins, NUM_BINS);
