...
__global__ void convolution_2D_kernel(float *P, float *N, int width, int height, int channels, const float* __restrict__ M) {
    ...
    for (int c = 0; c < channels; ++c) {
        if ((row_i >= 0) && (row_i < height) && (col_i >= 0) && (col_i < width)) {
            Ns[ty][tx][c] = P[(row_i * width + col_i) * channels + c];
        }
        else {
            Ns[ty][tx][c] = 0.0f;
        }
    ...
    __syncthreads();
    if (ty < O_TILE_WIDTH && tx < O_TILE_WIDTH && row_o < height && col_o < width) {
        for (int c = 0; c < channels; ++c) {
            float output = 0.0f;
            for (int i = 0; i < MASK_WIDTH; ++i) {
                for (int j = 0; j < MASK_WIDTH; ++j) {
                    output += M[i * MASK_WIDTH + j] * Ns[ty + i][tx + j][c];
                }
            }
            N[(row_o * width + col_o) * channels + c] = clamp(output);
        }
    }
}
